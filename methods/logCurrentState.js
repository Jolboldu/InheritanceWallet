...
const walletAddress = process.env.OWNER_WALLET_ADDRESS;
const walletPrivateKey = process.env.OWNER_WALLET_PRIVATE_KEY;
...
const log = async() => {
    ...
    const gasLimit = await sepoliaContract.methods.logCurrentState().estimateGas({ from: walletAddress });
    const gasLimitWithBuffer = gasLimit * 2n;
    const adjustedGasPrice = baseGasPrice * 2n
    
    const tx = {
        data: sepoliaContract.methods.logCurrentState().encodeABI(),
        from: walletAddress,
        value:0,
