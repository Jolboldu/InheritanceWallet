...
const heirWalletAddress = process.env.HEIR_WALLET_ADDRESS;
const heirWalletPrivateKey = process.env.HEIR_WALLET_PRIVATE_KEY;

const web3 = new Web3(new Web3.providers.HttpProvider(endpoint))

const claimOwnership = async() => {
    console.log('starting claiming ownership')
    const sepoliaContract = new web3.eth.Contract(contractABI, contractAddress);
    
    const baseGasPrice = await web3.eth.getGasPrice();
    const gasLimit = await sepoliaContract.methods.claimOwnership().estimateGas({ from: heirWalletAddress });
    const gasLimitWithBuffer = gasLimit * 2n;
    const adjustedGasPrice = baseGasPrice * 2n

    const tx = {
        data: sepoliaContract.methods.claimOwnership().encodeABI(),
        from: heirWalletAddress,
        value:0,
    ...
    const signedTx = await web3.eth.accounts.signTransaction(tx, heirWalletPrivateKey);
